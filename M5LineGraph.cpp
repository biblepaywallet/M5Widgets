...
void M5LineGraph::addValues(const float* values) {
  ...
  if (autoClearDisabled || !isDrawn())
    M5.Lcd.drawFastVLine(x+_pos+1, y+1, height-2, this->color(BACKGROUND));
  // Draw the line
  for (unsigned char i = 0; i < _dim; i++) {
    ...
    if (_pos && (*y_old_ptr != y_new)) {
      M5.Lcd.drawLine(x+x_new, y+*y_old_ptr, x+x_new, y+y_new, color);
    } else {
      M5.Lcd.drawPixel(x+x_new, y+y_new, color);
    }
    // Update old position
    *y_old_ptr = y_new;
  }
...
void M5LineGraph::draw() {
  // Draw the frame
  M5.Lcd.drawRect(x, y, width, height, color(FRAME));
  ...
  for (int dx = _pos; dx < (width - 2); dx++) {
    M5.Lcd.drawFastVLine(x + dx + 1, y + 1, height - 2, bg);
  }
}
