...
void M5Widget::setFont(const GFXfont* font) {
  this->myFont = font;
  M5.Lcd.setFont(font);
}
...
void M5Widget::undraw() {
  M5.Lcd.fillRect(x, y, width+1, height+1, color(BACKGROUND));
}
...
void drawStringMiddle(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const char* text, const GFXfont* myFont, uint16_t bg, uint16_t fg) {
  ...
  M5.Lcd.setTextColor(bg, fg);
  int16_t tw = M5.Lcd.textWidth(text);
  int16_t th = M5.Lcd.fontHeight(1);
  M5.Lcd.drawString(text, x+(width - tw) / 2, y+(height-th)/2 - 6, 1);
}
...
void M5Widget::fillBorderRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, uint16_t borderColor) {
  M5.Lcd.drawRect(x, y, w, h, borderColor);
  M5.Lcd.fillRect(x+1, y+1, w-2, h-2, color);
}
...
void M5Widget::fillBorderRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t radius, uint16_t color, uint16_t borderColor) {
  M5.Lcd.fillRoundRect(x, y, w, h, radius, color);
...
void M5Widget::drawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) {
  ...
  M5.Lcd.drawLine(x2, y2, x0, y0, color); 
}
